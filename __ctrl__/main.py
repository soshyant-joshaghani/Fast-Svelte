#!/usr/bin/env python3
"""fast-svelte SSH control — setup, clone, env, start/stop, update/reset, ACME.

Standalone (not foxg-ctrl). Run from this folder: fast-svelte-ctrl.bat …
"""
...
from utils.app.cli import build_app_subparser
from utils.dev.cli import build_dev_subparser, cmd_setup_local
from utils.prod.cli import build_prod_subparser
from utils.test.cli import build_test_subparser
...
def _add_target(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        ...
        nargs="?",
        default="fast-svelte",
        help="server id (default: fast-svelte) or 'all'",
    )
    p.add_argument(
        "--all-disabled",
        action="store_true",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="fast-svelte-ctrl",
        description=(
            "Single CLI for fast-svelte: local dev, tests, prod helpers, and SSH VM ops."
        ),
        ...
        epilog=(
            ...
            "\n"
            "SSH (VM):\n"
            "  fast-svelte-ctrl.bat setup | clone | env | start | stop | update\n"
            "  fast-svelte-ctrl.bat backup-acme | status | connect"
        ),
    )
    ...
    p = sub.add_parser(
        "setup-local",
        help="Create/refresh project .venv + npm install",
    )
    p.add_argument(
        "--force",
        action="store_true",
        help="Delete .venv and node_modules before installing",
    )
    ...
    for name, help_, fn in [
        ...
        ("clone", "Clone / pull the fast-svelte repo on the VM", cmd_clone),
    ...
    build_test_subparser(sub)
    build_prod_subparser(sub)
    build_app_subparser(sub)
    return parser


def _repl() -> int:
    parser = build_parser()
    echo("fast-svelte-ctrl> (setup-local | dev run all | test all | list | quit)")
    while True:
        try:
            line = input("fast-svelte-ctrl> ").strip()
